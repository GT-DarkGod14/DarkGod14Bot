...
from DarkGod14Bot import DB_NAME, OWNER_ID, dispatcher, LOGGER as log, BACKUP_PASS
...
def backup_db(_: CallbackContext):
    ...
    if not os.path.exists('{}/{}'.format(bkplocation, dbbkpname)):
        ...
        if os.path.exists('DarkGod14Bot/config.py'):
            print("config copied")
            shutil.copyfile('DarkGod14Bot/config.py', '{}/config.py'.format(bkplocation))
...
from DarkGod14Bot import updater as u
